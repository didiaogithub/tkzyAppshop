...
static CGFloat const kDefaultCloseButtonPadding = 10.0f;
...
@implementation FFPopTool

+ (FFPopTool *)sharedInstance {
    static FFPopTool *sharedClient = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedClient = [[FFPopTool alloc]init];
    });
    return sharedClient;
}
...
- (void)setCloseButtonType:(ButtonPositionType)closeButtonType {
    ...
    if (closeButtonType == ButtonPositionTypeNone) {
        ...
        if(closeButtonType == ButtonPositionTypeBottom){

//            closeFrame.origin.x = round(CGRectGetMidX(self.containerView.bounds) - kDefaultCloseButtonPadding);
            
            closeFrame.origin.x =  CGRectGetMidX(self.containerView.bounds) - closeFrame.size.width*0.5;
...
- (void)showWithPresentView:(UIView *)presentView animated:(BOOL)animated {
    ...
    [closeButton setImage:[UIImage imageNamed:@"activeClose"] forState:UIControlStateNormal];
    ...
    if(self.closeButtonType == ButtonPositionTypeBottom){
        CGRect closeFrame = closeButton.frame;
        closeFrame.origin.x =  CGRectGetMidX(containerView.bounds) - closeFrame.size.width*0.5;
        closeFrame.origin.y =  CGRectGetHeight(containerView.bounds);
        closeButton.frame = closeFrame;
    }
...
- (void)drawRect:(CGRect)rect{
    ...
    if ([FFPopTool sharedInstance].shadeBackgroundType == ShadeBackgroundTypeSolid) {
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    if ([FFPopTool sharedInstance].tapOutsideToDismiss) {
        [[FFPopTool sharedInstance] hideAnimated:YES withCompletionBlock:nil];
    }
}
