...
-(void)createCollectedListData{
    ...
    [HttpTool getWithUrl:districtUrl params:paraDic success:^(id json) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSString *areaCodeS = [self.areacodeS stringByAppendingString:[NSString stringWithFormat:@",%@",districtModel.code]];
    for (UIViewController *controller in self.navigationController.viewControllers) {
        
        if([controller isKindOfClass:[AddAddressViewController class]]){
            [KUserdefaults setObject:areaCodeS forKey:@"areaCodeS"];
