...
- (instancetype)initWithFrame:(CGRect )frame segmentStyle:(ZJSegmentStyle *)segmentStyle delegate:(id<ZJScrollPageViewDelegate>)delegate titles:(NSArray *)titles titleDidClick:(TitleBtnOnClickBlock)titleDidClick {
    if (self = [super initWithFrame:frame]) {
        ...
        [self setBackgroundColor:[UIColor tt_gradientTitleColor]];
...
#pragma mark - 此处为自己添加的代码
    _lineView = [UIView new];
    _lineView.backgroundColor = [UIColor tt_lineBgColor];
    [self addSubview:_lineView];
    [_lineView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.mas_offset(0);
        make.bottom.mas_offset(-AdaptedHeight(0.5));
...
- (void)reloadTitlesWithNewTitles:(NSArray *)titles andIndex:(NSInteger)index{
...
- (UIScrollView *)scrollView {
    ...
    if (!_scrollView) {
        ...
        [_scrollView setBackgroundColor:[UIColor tt_gradientTitleColor]];
    }
    return _scrollView;
}
